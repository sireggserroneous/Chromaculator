#!/usr/bin/env node
// mkcorpus7.js -- deterministic corpus for the v7 tournament.
...
// program.exe (the then-current eggv7 build; a real PE either way).
...
const root = path.join(here, '..');          // codegg-v7/
...
// 4. program.exe -- the then-current eggv7 build, a real PE
fs.copyFileSync(path.join(root, 'target', 'release', 'eggv7.exe'), path.join(out, 'program.exe'));
