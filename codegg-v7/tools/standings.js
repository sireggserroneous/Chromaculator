/* node codegg-v7/tools/standings.js <file>... -- the tournament, v7 row added.
 *
 ...
 *
 * Rows: the four compressors; eggv6 (armor only, the prior champion); the
 * eggv6+zstd hybrid (the old recommended posture); eggv7 (the transmuter --
 * its own dyadic stage inside its own armor, no borrowed compressor). */
const fs = require("fs"), zlib = require("zlib"), cp = require("child_process"), path = require("path");
const here = path.dirname(__filename);
const SP = process.env.EGG_TMP || path.join(here, "..", "corpus");
...
const EGG7 = path.join(here, "..", "target", "release", "eggv7" + (process.platform === "win32" ? ".exe" : ""));
...
function attempt(name, damaged, at, len, kind, src){
  try {
    ...
    if(name === "gzip") out = zlib.gunzipSync(damaged);
      ...
      fs.writeFileSync(`${SP}/a.egg7`, damaged);
      const wound = kind === "scratch" ? ` --wound ${at}:${len}` : "";
      cp.execSync(`"${EGG7}" restore "${SP}/a.egg7" -o "${SP}/a.out7"${wound}`, {stdio: "pipe"});
      out = fs.readFileSync(`${SP}/a.out7`);
    }
    else {
...
const NAMES = ["gzip", "zstd", "brotli", "xz", "egg6", "egg6+zstd", "egg7"];
...
console.log("file                    orig B      gzip    zstd  brotli      xz    egg6 e6+zstd    egg7   WINNER (size)");
console.log("-".repeat(120));
const tally = {};
let vsGzip = 0, structured = 0;
for(const file of files){
  ...
  const cell = n => {
    ...
    return (v.ok ? pct : v.lied ? "LIED" : "dq").padStart(8);
  };
  ...
  /* the v7 victory ledger: vs gzip -9's UNARMORED weight, on structured files */
  const isStructured = !/photo\.bin|archive\.zst/.test(short);
  if(isStructured){
    structured++;
    if(verdicts.egg7.ok && verdicts.egg7.size < verdicts.gzip.size) vsGzip++;
  }
}
console.log("-".repeat(120));
console.log("podium: " + Object.entries(tally).sort((a, b) => b[1] - a[1])
  .map(([n, c]) => `${n} x${c}`).join(", "));
console.log(`victory ledger: egg7 (armor ON, all injuries EXACT) lighter than gzip -9 on ${vsGzip} of ${structured} structured files`
  + (vsGzip >= 5 ? " -- THE BAR (>=5) IS MET" : " -- the bar (>=5) is NOT met"));
