...
pub fn tokenize(src: &[u8]) -> Vec<Tok> {
    ...
    while pos < n {
        let (len, dist, gain) = best_at(src, &head, &prev, pos, &reps, price8);
        if len >= MIN_MATCH && gain > 0 {
            // one-step lazy (gzip's move): a better match one byte later wins
            if len < MAX_LAZY && pos + 1 < n {
                let (_, _, gain2) = best_at(src, &head, &prev, pos + 1, &reps, price8);
                if gain2 > gain {
