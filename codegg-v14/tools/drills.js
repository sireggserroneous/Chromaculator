...
function drill(name, contBuf, orig, wounds, expect) {
  const p = path.join(tmp, 'drill.egg14');
...
function battery(tag, extra) {
  for (const { f, label } of cases) {
    ...
    const cont = path.join(tmp, f + tag + '.egg14');
...
// Built on an identity-form file (the inner IS the bytes) at --tier 2048 (P = 4).
{
  ...
  const cont = path.join(tmp, 'twins.egg14');
...
// ---- flag variants on the small artifact ----
{
  ...
  {
    const cont = path.join(tmp, f + '.triple.egg14');
  ...
  {
    const cont = path.join(tmp, f + '.none.egg14');
  ...
  {
    const cont = path.join(tmp, f + '.t1024.egg14');
  ...
  {
    const cont = path.join(tmp, f + '.p9.egg14');
  ...
  {
    const cont = path.join(tmp, f + '.s64k.egg14');
...
// 3-bit storms x300 on the mid artifact (default placement)
{
  ...
  for (let i = 0; i < 300; i++) {
    const p = path.join(tmp, 'storm.egg14');
...
// nothing repairs)
{
  ...
  const src = path.join(corpus, f);
  const orig = fs.readFileSync(src);
  const cont = path.join(tmp, f + '.noarmor.egg14');
...
// inside its promise and refuse honestly beyond it, exactly like any other.
{
  ...
  if (!fs.existsSync(jsrc)) {
    ...
    const cont = path.join(tmp, 'peel.egg14');
    transmute(jsrc, cont);
    const g = info(cont);
    const c = fs.readFileSync(cont);
    ...
    for (const [name, buf] of hostiles) {
      ...
      const hc = path.join(tmp, 'hostile.egg14');
      let ok = false, detail = '';
      try {
        transmute(hp, hc);
        const hg = info(hc);
...
// 128 error rows are rank 1 and there is no span to place a position in.
{
  ...
  for (const tier of [256, 512, 1024, 2048, 4096]) {
    const cont = path.join(tmp, `${f}.t${tier}.egg14`);
...
// wound inside the promise must still restore exactly through the undo.
{
  ...
  for (const [f, id, param, label] of cases) {
    ...
    const orig = fs.readFileSync(src);
    const cont = path.join(tmp, `filt${id}_${param}.egg14`);
    let g = null;
    try {
...
// regular is not a test of a 2D context, it is a test of an LZ model.
{
  ...
  for (const [name, want, buf, wantFilter, label] of cases) {
    ...
    const cont = path.join(tmp, name + '.arm.egg14');
    let g = null;
    try {
      transmute(src, cont, []);
      g = info(cont);
