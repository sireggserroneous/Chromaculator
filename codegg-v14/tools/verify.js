/* node codegg-v10/tools/verify.js <file>... -- the independent audit.
 *
...
const SP = process.env.EGG_TMP || fs.mkdtempSync(path.join(os.tmpdir(), "egg13sp-"));
const EGG9 = path.join(here, "..", "target", "release", "eggv14" + (process.platform === "win32" ? ".exe" : ""));
...
for(const file of process.argv.slice(2)){
  ...
  cp.execSync(`"${EGG9}" transmute "${file}" -o "${SP}/w.egg14"`, {stdio: "pipe"});
  const art = fs.readFileSync(`${SP}/w.egg14`);
  const short = file.split(/[\\/]/).pop();
  console.log(`${short}  (egg13, ${(100 * art.length / src.length).toFixed(1)}% of original)`);
