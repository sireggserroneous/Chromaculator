...
//  (1) eggv13 (v11's armor v3 still in place) must produce BIT-IDENTICAL
...
//      EXACT through eggv13 (the four ancestors' forms, armor11.rs verbatim).
...
const v12exe = path.join(v12root, 'target', 'release', 'eggv14.exe');
...
async function gateFile(dir, f) {
  ...
  const a11 = path.join(tmp, f + '.egg11'), a12 = path.join(tmp, f + '.egg14');
  const row = { f, size };
  try {
