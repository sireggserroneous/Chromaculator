//! mix11 -- v11's live model, forked at M0 as a BYTE-COPY of mix10 (the
//! Keeper's frozen tier). Divergence begins at M1+; until then the inner
...
//! never-lose insurance). Mix11 is coder-free: it predicts and learns,
...
const LAT_SMAX: usize = 384; // largest record stride the lattice will look for
const LAT_LOCK1: usize = 1 << 16; // first deterministic lock attempt
const LAT_LOCK2: usize = 1 << 18; // final attempt; past this the counter is off
...
// wider.
const ISSE_LR: u32 = 9; // v11-M8 sweep: notepad -1,524 with arial/icon/zstd following; kernel32 +512 is the printed cost
...
impl StateMap {
    /// v11-M5 export surface: the raw (p22<<10 | count) entries
    pub fn dump(&self) -> &[u32] {
        &self.t
    }
    /// v11-M5 import: trained probabilities, counts capped so a divergent
    /// file can still move the map early (the guess is educated, not sworn)
...
#[allow(dead_code)] // vestigial pre-ISSE fast counter; removal scheduled for the first M1 edit
#[inline]
...
pub struct Mix11 {
    ...
    #[allow(dead_code)] // vestigial pre-ISSE map (chain replaced its read); M1 cleanup
    sm2: StateMap,
    ...
    #[allow(dead_code)] // vestigial, as sm2
    sm3: StateMap,
    #[allow(dead_code)] // vestigial, as sm2
    sm4: StateMap,
    #[allow(dead_code)] // vestigial, as sm2
    ...
    smi1: StateMap,
    smi2: StateMap,
    // v11-M3, the lattice reading (stalk.js:211-215: "A across the columns,
    // B down the rows"): record-structured bytes form a grid and these two
    // tables read DOWN the columns -- contexts keyed on b[-s] and
    // (b[-s], b[-2s]) for a stride s LEARNED from the bytes. Both sides run
    // the same detector inside byte_update (the one per-byte mover), so the
    // mirror holds by construction. Until a stride locks, both inputs are
    // constant zero AND their mixer weights start at zero, so every
    // unlocked stream is bit-identical to the pre-lattice build.
    lat_cnt: Vec<u32>, // stride hit counters, index = stride, 3..=LAT_SMAX
    lat_state: u8,     // 0 counting, 1 locked, 2 off
    lat_s: u32,        // the locked stride
    lat_cl1: usize,    // cached column contexts for the NEXT byte
    lat_cl2: usize,
    lat1s: Vec<u8>, // 2^18 x 16 states
    lat2s: Vec<u8>, // 2^18 x 16 states
    sml1: StateMap,
    sml2: StateMap,
    lr: u32, // mixer LR; MIX10_LR default, 13 in the heavy arms (MODEL 14/15)
...
pub struct Bit10 {
    ...
    wsel: usize,
    idx: [usize; 14], // o0 o1s..ind2s mb mm lat1s lat2s
    smcx: [usize; 11], // node*256+state (chain + sparse + indirect + lattice)
    lat_on: bool,
...
impl Mix11 {
    pub fn new() -> Self {
        ...
        for v in w.chunks_mut(NINPUT) {
            // the nine veteran inputs keep v10's exact (1<<16)/9 blend; the
            // two lattice inputs and the bias start at ZERO -- unlocked
            // streams stay bit-identical by arithmetic, locked ones learn
            // the new inputs in from nothing
    ...
    pub fn byte_update(&mut self, buf: &[u8], pos_new: usize) {
        ...
        // the lattice detector (v11-M3): count stride hits over the head of
        // the stream, lock deterministically, then cache the column contexts
        // for the byte about to be coded. Encoder and decoder run this on
        // the same bytes at the same positions -- mirror by construction.
        let p = pos_new;
        match self.lat_state {
            0 => {
                ...
                if p == LAT_LOCK1 || p == LAT_LOCK2 {
                    ...
                    // harmonic fold: the smallest stride within 85% of the peak
                    for st in 3..bs {
                    ...
                    if std::env::var_os("EGG_STATEHASH").is_some() {
                        eprintln!(
                            "lattice probe p={} best_s={} bc={} avg={} n/8={}",
                            p, bs, bc, avg, n / 8
                        );
                    }
                    if bc > avg.saturating_mul(3) && bc > n / 8 && bs >= 3 {
                        ...
                        self.lat_state = 2; // off; the counter never runs again
                    }
                }
            }
            1 => {}
        ...
        if self.lat_state == 1 {
            ...
            if p >= 2 * st {
                ...
                let k1 = a | (0xA3u64 << 56);
                self.lat_cl1 =
                    (((k1.wrapping_mul(0x9E3779B97F4A7C15) >> 46) as usize) & 0x3ffff) * 16;
                let k2 = a | (b2 << 8) | (0xA4u64 << 56);
                self.lat_cl2 =
                    (((k2.wrapping_mul(0x9E3779B97F4A7C15) >> 46) as usize) & 0x3ffff) * 16;
            } else {
                self.lat_cl1 = 0;
                self.lat_cl2 = 0;
            }
    ...
    pub fn predict(
        ...
        let ps = [
            ...
            if lat_on { self.sml1.p(smcx[9]) } else { 2048 },
            if lat_on { self.sml2.p(smcx[10]) } else { 2048 },
        ];
        let mut sts = [0i32; NINPUT - 1];
        for k in 0..NINPUT - 1 {
        ...
        let mut sts = {
            let mut all = [0i32; NINPUT];
            all[..NINPUT - 1].copy_from_slice(&sts);
            all
        };
        ...
        let mut t: i64 = 0;
        for (&wv, &st) in self.w[wsel..wsel + NINPUT].iter().zip(sts.iter()) {
    ...
    pub fn learn(&mut self, b9: &Bit10, b: u32) {
        ...
        for (w, &st) in self.w[b9.wsel..b9.wsel + NINPUT].iter_mut().zip(b9.sts.iter()) {
            *w = (*w + ((err * st) >> self.lr)).clamp(-WCLAMP, WCLAMP);
        }
    ...
    /// v11-M5, the free-to-guess reading (glossary.js:104: "The zero cells.
    /// They have no sign of their own, so the page is free to guess one."):
    /// tables seeded from the site's own book -- the convention both halves
    /// already carry. Identical constants on both sides: mirror by build.
    ...
    /// emit prior_tab.rs from a seasoned model (gen-prior subcommand)
    pub fn export_prior(&self) -> String {
        self.export_book("PRIOR", "//! GENERATED by `eggv11 gen-prior` from the site's book (the test page\n//! excluded). Regenerate and byte-compare; never edit.\n")
    }
    /// v12-M2c(c): the same export under another name -- a dialect book
    /// (PE, TTF) trained on files that are in NO corpus of this repo
    pub fn export_book(&self, prefix: &str, header: &str) -> String {
        let mut out = String::with_capacity(1 << 22);
        out.push_str(header);
        out.push_str(&format!("pub const {}_ON: bool = true;\n", prefix));
        let dump_i32 = |out: &mut String, name: &str, v: &[i32]| {
            out.push_str(&format!("pub static {}: [i32; {}] = [", name, v.len()));
            for x in v {
                out.push_str(&format!("{},", x));
            }
            out.push_str("];\n");
        };
        let dump_u8 = |out: &mut String, name: &str, v: &[u8]| {
            out.push_str(&format!("pub static {}: [u8; {}] = [", name, v.len()));
            for x in v {
                out.push_str(&format!("{},", x));
            }
            out.push_str("];\n");
        };
        dump_i32(&mut out, &format!("{}_W", prefix), &self.w);
        dump_u8(&mut out, &format!("{}_H1", prefix), &self.h1);
        dump_u8(&mut out, &format!("{}_H2", prefix), &self.h2);
        dump_u8(&mut out, &format!("{}_O1S", prefix), &self.o1s);
        out.push_str(&format!("pub static {}_SM1: [u32; {}] = [", prefix, self.sm1.dump().len()));
        for x in self.sm1.dump() {
            out.push_str(&format!("{},", x));
        }
        out.push_str("];\n");
        out
...
mod tests {
    ...
    fn state_tab_invariants() {
        for row in NEX.iter().take(NSTATE) {
            assert!((row[0] as usize) < NSTATE && (row[1] as usize) < NSTATE);
        }
        let mut s = 0usize;
        for _ in 0..20 {
            s = NEX[s][0] as usize;
