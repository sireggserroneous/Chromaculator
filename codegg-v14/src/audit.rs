...
/// wound PART of each listed square: `nbytes` bytes of noise at a random offset
/// inside it. A partly-hit square is an ERROR, not an erasure -- its position is
/// not handed to the decoder, so the syndromes must locate it -- and several
/// bytes are used rather than one so that several of the interleaved codewords
/// see it, which is the case `consistent()` is there to judge.
fn hurt_squares(cont: &[u8], g: &Geom, squares: &[usize], nbytes: usize, seed: u64) -> Vec<u8> {
    let mut hurt = cont.to_vec();
    let mut st = seed | 1;
    for &j in squares {
        let o = square_off(g, j);
        let l = square_len(g, j);
        let k = nbytes.min(l);
        let at = if l > k { (xorshift(&mut st) as usize) % (l - k + 1) } else { 0 };
        for i in 0..k {
            let b = &mut hurt[o + at + i];
            let was = *b;
            *b = xorshift(&mut st) as u8;
            if *b == was {
                *b ^= 0x5A;
...
pub fn run(full: bool) -> bool {
    ...
    c.section();
    // ---------- (i) MIXED DAMAGE: k fully dead + m partly hit ----------
    // v14-N1. The drill comes first: nothing else in this file wounds a
    // container both ways at once, and that combination is the only case in
    // which `consistent()` decides anything. A fully dead square is an ERASURE
    // (position known, costs 1 of t); a partly hit square is an ERROR (position
    // unknown, costs 2 of t). The classical bound is 2m + k <= t, so every
    // trial below is labelled REPAIRABLE or BEYOND by that arithmetic and the
    // drill reports the two classes separately.
    //
    // What it may NOT do, ever: return wrong bytes under a success code. What
    // it is hunting: a REPAIRABLE wound this build refuses.
    let mut i_trials = 0u64;
    let mut i_exact = 0u64;
    let mut i_refused_repairable = 0u64;
    let mut i_refused_beyond = 0u64;
    let mut i_exact_beyond = 0u64;
    let mut i_hash_caught = 0u64;
    let mut i_wrong = 0u64;
    let mut worst: Vec<String> = Vec::new();
    for cs in cases.iter().filter(|cs| cs.t >= 3) {
        let inner = xbytes(cs.len, cs.len as u64 * 29 + 7);
        let g = geom(cs.len, cs.blk, cs.t, cs.mode, fnv64(&inner), dummy_ex());
        if g.n > NMAX || g.n <= cs.t + 2 {
            continue;
        }
        let cont = armor(&inner, cs.blk, cs.t, cs.mode, dummy_ex());
        let t = cs.t;
        let mut st = 0x11CE_u64 ^ (cs.len as u64) << 3 ^ cs.blk as u64;
        // sweep the mix: m errors and k erasures, across and past 2m + k = t
        for m in 1..=((t / 2) + 1).min(4) {
            for k in 0..=(t.saturating_sub(2 * m) + 2).min(6) {
                if k + m + 1 >= g.n {
                    continue;
                }
                for &nbytes in &[1usize, 3, 9] {
                    if !full && nbytes != 3 {
                        continue;
                    }
                    let picks = pick_distinct(g.n, k + m, &mut st);
                    let (dead, part) = picks.split_at(k);
                    let (hurt, wounds) = kill_squares(&cont, &g, dead, xorshift(&mut st));
                    let hurt = hurt_squares(&hurt, &g, part, nbytes, xorshift(&mut st));
                    let repairable = 2 * m + k <= t;
                    // the dead squares are NAMED (that is what an erasure is);
                    // the partly hit ones never are
                    i_trials += 1;
                    match dearmor(&hurt, &wounds, true) {
                        Ok(o) => {
                            if !o.hash_ok {
                                ...
                                i_wrong += 1;
                                c.check(false, || format!("(i) MIXED k={k} m={m} nb={nbytes} blk {} {} len {}: SUCCESS CODE WITH WRONG DATA", cs.blk, cs.mode.name(), cs.len));
                            } else if repairable {
                                i_exact += 1;
                            } else {
                                i_exact_beyond += 1;
                        ...
                        Err(e) => {
                            if repairable {
                                i_refused_repairable += 1;
                                if worst.len() < 12 {
                                    worst.push(format!("k={k} m={m} nbytes={nbytes} t={t} blk={} {} len={} -> {}", cs.blk, cs.mode.name(), cs.len, e.chars().take(70).collect::<String>()));
                                }
                            } else {
                                i_refused_beyond += 1;
                            }
                            c.check(e.contains("capacity") || e.contains("dead squares") || e.contains("refus") || e.contains("inconsistent") || e.contains("locate"), || format!("(i) refusal without a number: {e}"));
                        }
                    }
                }
            }
    ...
    // conservation is the only thing this section may not survive; reach is what
    // it MEASURES, so a repairable refusal is reported, not asserted away
    c.check(i_wrong == 0, || format!("(i) {i_wrong} mixed wounds returned WRONG DATA under a success code"));
    println!("  (i) MIXED damage (k dead + m partly hit, the case nothing else generates): {} trials; within 2m+k<=t: {} EXACT and {} REFUSED; beyond: {} refused, {} exact anyway, {} caught by the hash; WRONG DATA: {}", i_trials, i_exact, i_refused_repairable, i_refused_beyond, i_exact_beyond, i_hash_caught, i_wrong);
    if i_refused_repairable > 0 {
        println!("      REPAIRABLE WOUNDS THIS BUILD REFUSES -- the reach gap N1 is hunting:");
        for w in &worst {
            println!("        {w}");
        }
    } else {
        println!("      no repairable mixed wound was refused: the reach is intact on this sweep");
    }

    c.section();
    // ---------- (j) THE FALSE-ACCEPT HUNT ----------
    // v14-N1. Section (i) showed the subset search is never entered: the joint
    // locator returned exactly its rank every time. It is entered only when the
    // locator over-reports, whose rate is ~n/65536^(m-k), so this section makes
    // both factors as favourable as a container allows -- n near NMAX, and a
    // RANK-1 dependency (two identical squares wounded identically) so
    // m - k = 1 rather than 2, where the rate is 65,536x smaller.
    let mut j_trials = 0u64;
    let mut j_over = 0u64;
    let mut j_exact = 0u64;
    let mut j_refused = 0u64;
    let mut j_hash = 0u64;
    let mut j_wrong = 0u64;
    let mut j_note: Vec<String> = Vec::new();
    {
        let blk = 256usize;
        // placement "none": the mode whose whole mechanism IS the joint locator
        let mode = CtMode::Absent;
        let t = mode.parity_for(blk, SURVIVE_DEFAULT);
        // walk the length up until n is just under NMAX: that is where the
        // locator's false-positive rate is highest
        let mut best: Option<(usize, usize)> = None;
        for mb in [1usize, 2, 4, 8, 12, 16] {
            let len = mb * 1_000_000;
            let probe = xbytes(64, 1);
            let _ = &probe;
            let gg = geom(len, blk, t, mode, 0, dummy_ex());
            if gg.n <= NMAX {
                best = Some((len, gg.n));
            }
        }
        if let Some((len, nsq)) = best {
            let inner_base = xbytes(len, 0xA11CE);
            for trial in 0..(if full { 24 } else { 6 }) {
                let mut st = 0x5EED_u64 ^ ((trial as u64) * 0x9E37_79B9);
                // the rank-1 dependency: two DATA squares made identical
                let inner = inner_base.clone();
                let nd = len / blk;
                if nd < 8 {
                    break;
                }
                // THE ARITHMETIC IS THE AIM, and getting it wrong cost two
                // rounds of this drill. In `decode`, m = t - |E|, so a blind
                // wound has m = t = 18, and k = rank(rows) = the number of
                // DISTINCT damaged positions. The locator's false-positive rate
                // is ~n/65536^(m-k), so:
                //   * w = 10 wounds  -> k = 10, m - k = 8  -> n/65536^8, hopeless
                //   * w = t - 1 = 17 -> k = 17, m - k = 1  -> n/65536 ~ 0.95
                // and a rank-DEFICIENT wound (the twin trick that section (h)
                // uses) makes it WORSE, not better, because it lowers k. So the
                // wounds here are w = t - 1 INDEPENDENT squares -- also exactly
                // the edge the joint locator is claimed to reach (e <= t - 1).
                let m_err = t - 1 - (trial as usize % 2);
                let picks = pick_distinct(nd.saturating_sub(4).max(2), m_err, &mut st);
                let (a, b) = (picks[0].max(1), picks[1].max(1));
                let g = geom(inner.len(), blk, t, mode, fnv64(&inner), dummy_ex());
                if g.n > NMAX {
                    break;
                }
                let cont = armor(&inner, blk, t, mode, dummy_ex());
                // wound BOTH twins with the SAME noise -> identical error rows
                let noise = xbytes(blk, xorshift(&mut st));
                let mut hurt = cont.clone();
                let mut placed = 0;
                for (i, &sq) in picks.iter().enumerate() {
                    let o = square_off(&g, sq);
                    let l = square_len(&g, sq);
                    if l != blk || o + l > hurt.len() {
                        continue;
                    }
                    // every wound INDEPENDENT: full rank k = w, so m - k is as
                    // small as the geometry allows
                    let _ = (i, &noise);
                    let ind = xbytes(l, xorshift(&mut st));
                    hurt[o..o + l].copy_from_slice(&ind);
                    placed += 1;
                }
                if placed != picks.len() {
                    continue;
                ...
                j_trials += 1;
                let before = crate::armor::FOUND_GT_K.load(std::sync::atomic::Ordering::Relaxed);
                match dearmor(&hurt, &[], true) {
                    Ok(o) => {
                        if !o.hash_ok {
                            j_hash += 1;
                            ...
                            j_wrong += 1;
                            c.check(false, || format!("(j) FALSE ACCEPT WITH WRONG DATA at n={nsq} blk={blk}"));
                        } else {
                            j_exact += 1;
                        }
                    }
                    Err(e) => {
                        j_refused += 1;
                        if j_note.len() < 6 {
                            j_note.push(format!("trial {trial} m={m_err}: REFUSED -- {}", e.chars().take(90).collect::<String>()));
                        }
                    }
                }
                let after = crate::armor::FOUND_GT_K.load(std::sync::atomic::Ordering::Relaxed);
                if after > before {
                    j_over += 1;
                    if j_note.len() < 6 {
                        j_note.push(format!("trial {trial}: locator OVER-REPORTED at n={nsq}, w={m_err} (first two {a},{b})"));
                    }
                }
            }
            println!("  (j) FALSE-ACCEPT HUNT at n={} squares, blk={}, t={}, w = t-1 INDEPENDENT wounds so m-k = 1: {} trials; locator OVER-reported {} times; {} exact, {} refused, {} caught by the hash; WRONG DATA: {}", nsq, blk, t, j_trials, j_over, j_exact, j_refused, j_hash, j_wrong);
        } else {
            println!("  (j) FALSE-ACCEPT HUNT: no length put n under NMAX; not run");
        }
    }
    c.check(j_wrong == 0, || format!("(j) {j_wrong} false accepts returned WRONG DATA under a success code"));
    for w in &j_note {
        println!("        {w}");
    }
    if j_over == 0 {
        println!("      the locator never over-reported even here: the subset search remains unreachable, and the ~32-false-accept claim is UNTESTABLE on this armor rather than untested");
    } else {
        for w in &j_note {
            println!("        {w}");
        }
    ...
    if std::env::var_os("EGG_CONSIST").is_some() {
        use std::sync::atomic::Ordering::Relaxed;
        let (calls, bad, read, rej) = (
            crate::armor::CONSIST_CALLS.load(Relaxed),
            crate::armor::CONSIST_BAD.load(Relaxed),
            crate::armor::CONSIST_READ.load(Relaxed),
            crate::armor::CONSIST_REJECT.load(Relaxed),
        );
        println!(
            "  consistency check REACHED {} times; handed {} bad codewords, READ {} of them ({:.1}%); rejected {}",
            calls, bad, read,
            if bad == 0 { 0.0 } else { 100.0 * read as f64 / bad as f64 },
            rej
        );
        println!(
            "  reached VIA the partial-location path {} times, VIA the brute subset search {} times; confirm_subset entered {} times and accepted {}",
            crate::armor::CONSIST_VIA_PARTIAL.load(Relaxed),
            crate::armor::CONSIST_VIA_BRUTE.load(Relaxed),
            crate::armor::CONFIRM_SUBSET_CALLS.load(Relaxed),
            crate::armor::CONFIRM_SUBSET_HITS.load(Relaxed)
        );
        println!(
            "  the joint locator returned MORE candidates than its rank {} times (max excess {}) and exactly its rank {} times; the subset search tried {} subsets",
            crate::armor::FOUND_GT_K.load(Relaxed),
            crate::armor::FOUND_EXCESS_MAX.load(Relaxed),
            crate::armor::FOUND_EQ_K.load(Relaxed),
            crate::armor::SUBSET_TRIES.load(Relaxed)
        );
        let runs = crate::armor::LOC_RUNS.load(Relaxed).max(1);
        println!(
            "  the locator ran {} times: mean m = {:.1}, mean rank k = {:.1}, mean m-k = {:.1} (SMALLEST m-k seen: {}), mean n = {:.0}; expected false positives per run at the smallest m-k ~ n/65536^(m-k)",
            crate::armor::LOC_RUNS.load(Relaxed),
            crate::armor::LOC_M_SUM.load(Relaxed) as f64 / runs as f64,
            crate::armor::LOC_K_SUM.load(Relaxed) as f64 / runs as f64,
            (crate::armor::LOC_M_SUM.load(Relaxed) as f64 - crate::armor::LOC_K_SUM.load(Relaxed) as f64) / runs as f64,
            crate::armor::LOC_MK_MIN.load(Relaxed),
            crate::armor::LOC_N_SUM.load(Relaxed) as f64 / runs as f64
        );
    }
