...
/// the model trial: v7's two depths AND the v8 mixer, in parallel threads;
/// the lightest point wins and the model byte remembers the walk. The
/// insurance the plan demanded: a regression against v7 is impossible by
/// construction, because v7's exact paths are two of the three entrants.
...
fn filtered_transmute(src: &[u8], forced: Option<(u8, u32)>) -> (Vec<u8>, u8, u8, u32) {
    ...
    if cands.is_empty() {
        let (inner, model) = enc(src);
        return (inner, model, 0, 0);
    }
    let filtered_srcs: Vec<Vec<u8>> = cands.iter().map(|c| filter::apply(src, c.id, c.param)).collect();
    let (plain, trials) = std::thread::scope(|s| {
        let handles: Vec<_> = filtered_srcs.iter().map(|f| s.spawn(|| enc(f))).collect();
        let plain = enc(src);
        let trials: Vec<(Vec<u8>, u8)> = handles.into_iter().map(|h| h.join().expect("trial thread")).collect();
        (plain, trials)
    });
    let mut best: Option<usize> = None; // index into trials; None = plain
    for (i, t) in trials.iter().enumerate() {
        let cur = best.map(|b| trials[b].0.len()).unwrap_or(usize::MAX);
        if t.0.len() as u64 * 1000 <= plain.0.len() as u64 * 995 && t.0.len() < cur {
            best = Some(i);
        }
    }
    match best {
        Some(i) => {
            let (inner, model) = trials.into_iter().nth(i).unwrap();
            (inner, model, cands[i].id, cands[i].param)
        }
        None => (plain.0, plain.1, 0, 0),
    }
}
...
fn main() -> ExitCode {
    ...
    let usage = || {
        eprintln!("usage: eggv8 transmute <file> [-o out.egg8] [--no-armor] [--group N] [--parity T]");
        eprintln!("       eggv8 restore <file.egg8> [-o out] [--wound start:len] [--no-doubles]");
        eprintln!("       eggv8 scratch <file> [--len 4096] [--at payload|checks|head|end|<off>]");
        eprintln!("       eggv8 audit [--full]   -- the geometry audit, counts printed");
        ExitCode::from(2)
    };
    ...
    match cmd {
        "transmute" => {
            ...
            let (inner, model, fid, fparam) = if model_req == MODEL_DYADIC && !has("--stats") {
                ...
                println!("  stats: lit-ctx 2 nibs -> {} B, 4 nibs -> {} B, mixed -> {} B (keeping the lightest)",
                    out8.len(), out16.len(), outmx.len());
                if outmx.len() <= out8.len() && outmx.len() <= out16.len() {
            ...
            let dst = get("-o").unwrap_or(format!("{}.egg8", path));
        ...
        "restore" => {
            ...
            let dst = get("-o").unwrap_or_else(|| {
                path.strip_suffix(".egg8").unwrap_or(path).to_string() + ".out"
            });
