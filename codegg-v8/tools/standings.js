/* node codegg-v8/tools/standings.js <file>... -- the tournament, v8 row added.
 *
...
const SP = process.env.EGG_TMP || fs.mkdtempSync(path.join(os.tmpdir(), "egg8sp-"));
...
const EGG8 = path.join(here, "..", "target", "release", "eggv8" + (process.platform === "win32" ? ".exe" : ""));
...
function attempt(name, damaged, at, len, kind, src){
  try {
    ...
    else if(name === "egg7" || name === "egg8"){
      const exe = name === "egg8" ? EGG8 : EGG7;
...
const NAMES = ["gzip", "zstd", "brotli", "xz", "egg6", "egg6+zstd", "egg7", "egg8"];
...
console.log("file                    orig B      gzip    zstd  brotli      xz    egg6 e6+zstd    egg7    egg8   WINNER (size)");
console.log("-".repeat(128));
const tally = {};
let barGz = 0, barHybrid = 0, barXz = 0, total = 0;
for(const file of files){
  ...
  /* the v8 victory ledger: three bars, armor ON, all injuries EXACT */
  total++;
  if(verdicts.egg8.ok){
    if(verdicts.egg8.size < art._gzStar) barGz++;
    if(verdicts.egg8.size < verdicts["egg6+zstd"].size) barHybrid++;
    if(verdicts.egg8.size < verdicts.xz.size) barXz++;
  }
}
console.log("-".repeat(128));
console.log("podium: " + Object.entries(tally).sort((a, b) => b[1] - a[1])
  .map(([n, c]) => `${n} x${c}`).join(", "));
console.log(`victory ledger (egg8, armor ON, all injuries EXACT, of ${total} files):`);
console.log(`  bar i   vs strongest gzip -9 : ${barGz}/${total}  (needs >=10 of 12) ${barGz >= 10 ? "-- MET" : "-- not met"}`);
console.log(`  bar ii  vs egg6+zstd hybrid  : ${barHybrid}/${total}  (needs >=6 of 12) ${barHybrid >= 6 ? "-- MET" : "-- not met"}`);
console.log(`  bar iii vs xz -9             : ${barXz}/${total}  (needs >=3 of 12) ${barXz >= 3 ? "-- MET" : "-- not met"}`);
