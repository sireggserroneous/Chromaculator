/* node codegg-v8/tools/verify.js <file>... -- the independent audit.
 *
...
for(const file of process.argv.slice(2)){
  ...
  cp.execSync(`"${EGG8}" transmute "${file}" -o "${SP}/w.egg8"`, {stdio: "pipe"});
  const art = fs.readFileSync(`${SP}/w.egg8`);
  const short = file.split(/[\\/]/).pop();
  console.log(`${short}  (egg8, ${(100 * art.length / src.length).toFixed(1)}% of original)`);
  ...
  for(const kind of ["flip", "scratch", "trunc"]){
    ...
    try {
      cp.execSync(`"${EGG8}" restore "${SP}/w.hurt" -o "${rec}"${wound}`, {stdio: "pipe"});
      const h = sha256(rec);
      const ok = h === origHash;
      if(!ok) all = false;
