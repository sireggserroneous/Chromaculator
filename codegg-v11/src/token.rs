...
const BUDGET_MIN: usize = 1 << 24; // the cliff budget arms only above 16 MB:
                                   // below it, hb stays 18 and the walk is
                                   // untouched -- those artifacts must stay
                                   // bit-identical to v10's (the M2 control)
...
/// the under-a-pixel reading (atlas.html:255: "under a pixel the split is
/// worse than useless"): on big inputs the 2^18 buckets crowd until most of
/// a 1024-step walk is colliding 4-grams -- work past usefulness. The bucket
/// count scales with the input (chains hold ~40 true candidates) and a work
/// budget halves the walk cap wherever density still saturates it.
#[inline]
fn hash_bits_for(n: usize) -> u32 {
    (usize::BITS - n.leading_zeros()).saturating_sub(6).clamp(18, 23)
}
#[inline]
fn hash4(src: &[u8], pos: usize, bits: u32) -> usize {
...
#[allow(clippy::too_many_arguments)] // the walk's whole state, spelled out
fn best_at(
    src: &[u8],
    head: &[u32],
    prev: &[u32],
    ...
    let mut c = head[hash4(src, pos, bits)];
    let mut chain = 0;
    while c != u32::MAX && chain < cap {
        let cp = c as usize;
...
pub fn tokenize_priced(src: &[u8], price8: i64) -> Vec<Tok> {
    ...
    let hb = hash_bits_for(n);
    let mut head = vec![u32::MAX; 1usize << hb];
    let mut prev = vec![u32::MAX; n];
    let mut reps: [u32; 4] = [0; 4];
    let mut pos = 0usize;
    // the cliff budget (armed only >= 16 MB): if the last 1 MiB of positions
    // averaged a saturated walk, halve the cap (floor 64); relax it back when
    // the density thins. Deterministic; encoder-only; tokens stay the format.
    let budget_on = n >= BUDGET_MIN;
    let mut cap = MAX_CHAIN;
    let mut steps: u64 = 0;
    let mut window_mark = 0usize;
    ...
    while pos < n {
        if budget_on && pos - window_mark >= (1 << 20) {
            let avg = steps / ((pos - window_mark) as u64);
            if avg > 128 {
                cap = (cap / 2).max(64);
            } else if avg < 24 && cap < MAX_CHAIN {
                cap *= 2;
            }
            steps = 0;
            window_mark = pos;
        ...
        let (len, dist, gain) = match c1.take() {
            ...
            None => best_at(src, &head, &prev, pos, &reps, price8, hb, cap, &mut steps),
        };
        if len >= MIN_MATCH && gain > 0 {
            if len < MAX_LAZY && pos + 1 < n {
                ...
                let b1 = match c2.take() {
                    ...
                    None => best_at(src, &head, &prev, pos + 1, &reps, price8, hb, cap, &mut steps),
                };
                if b1.2 > gain {
                ...
                if pos + 2 < n {
                    let b2 = best_at(src, &head, &prev, pos + 2, &reps, price8, hb, cap, &mut steps);
                    if b2.2 > gain + price8 {
                        toks.push(Tok::Lit(src[pos]));
                        insert!(pos);
                        pos += 1;
            ...
            #[allow(clippy::needless_range_loop)] // the macro writes head/prev AT p; an iterator obscures the chain
            for p in pos..pos + len {
                insert!(p);
            }
            pos += len;
        } else {
            c1 = c2.take(); // best_at(pos+1) if a probe left one
            toks.push(Tok::Lit(src[pos]));
            insert!(pos);
            pos += 1;
        }
