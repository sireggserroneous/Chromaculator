...
pub const BLOCK: usize = 512; // the v2 square (versions <= 4; tier 0 of v5)
// v11-M1, the wide-grid reading (spec.md:156 / stalk.js:283: "Widening the
// grid grows Q and shrinks R. The identity never moves"): squares widen with
// the payload, the residue table thins per byte, and every tier's moduli keep
// the same law -- +-2^k injective over the square's own bit count, asserted
// at table build and audited every run. Moduli stay u16 by construction, so
// the check-table entry is 4 bytes at every tier.
pub const TIERS: [(usize, u32, u32); 3] =
    [(512, 8219, 8221), (1024, 16421, 16427), (2048, 32771, 32783)];
pub const GMAX5: usize = 250; // v5 parity base: x = GMAX5 + t <= 253 in GF(256)
/// square size for a payload: <1 MB keeps v2's 512; 1-8 MB 1024; >=8 MB 2048
pub fn tier_for(len: usize) -> usize {
    if len < (1 << 20) { 0 } else if len < (8 << 20) { 1 } else { 2 }
}
pub fn tier_index(blk: usize) -> usize {
    TIERS.iter().position(|t| t.0 == blk).expect("unknown block tier")
}
/// most slots a contiguous 4096-byte scratch can straddle at this square size
...
fn c_of(s: usize, g: usize, t: usize, blk: usize) -> usize {
    if s == 0 {
        return 0;
    }
    let ng1 = s.div_ceil(g);
    let ct = 4 * (s + ng1 * t);
    if ct <= CT_TRIPLE_MAX {
        0
    } else {
...
/// does (G, T) guarantee that any contiguous 4 KB scratch (dead_slots(blk)
/// dead slots at this tier) leaves every group with <= T dead members? The
/// claim the audit proves, per tier.
pub fn guaranteed_st(s: usize, g: usize, t: usize, blk: usize) -> bool {
    if s == 0 || g == 0 {
        return false;
    }
    let ng_eff = ng_eff_of(s, c_of(s, g, t, blk), g, t);
    ng_eff > 0 && dead_slots(blk).div_ceil(ng_eff) <= t
}
...
pub fn rib_policy(inner_len: usize) -> Rib {
    ...
    let blk = TIERS[tier_for(inner_len)].0;
    let s = inner_len.div_ceil(blk);
    ...
    for g in 4..=(GMAX5 - 2) {
        for t in [2usize, 3] {
            if g + t > GMAX5 || !guaranteed_st(s, g, t, blk) {
                continue;
            }
            let total = geom(inner_len, blk, GMAX5, g, t, 0, dummy).total;
            let cand = (total, t, g);
    ...
    if let Some((_, t, g)) = capped.or(open) {
        return Rib { blk, g, t, guaranteed: true };
    }
    Rib { blk, g: 4, t: 3, guaranteed: false } // the physics floor, said out loud
}
...
pub struct Geom {
    pub len: usize,
    pub blk: usize,  // square size (the tier); 512 in every container <= v4
    pub gmax: usize, // RS parity x-base: 128 (<= v4), 250 (v5)
    pub g: usize,
    pub t: usize,
    pub s: usize,        // level-1 data squares
    #[allow(dead_code)] // geometry record kept complete: the info surface and future tiers read it
    ...
    #[allow(dead_code)] // geometry record kept complete, as p above
...
pub fn geom(len: usize, blk: usize, gmax: usize, g: usize, t: usize, hash: u64, ex: Extras) -> Geom {
    if g == 0 {
        return Geom {
            len, blk, gmax, g: 0, t: 0, s: 0, p: 0, nsq: 0, ct: 0, ct_triple: true,
            c: 0, p2: 0, nsq2: 0, m: 0, msize: 0, nslots: 0, mid: 0,
            hash, ex, total: 3 * HDR + len,
        };
    }
    let s = len.div_ceil(blk);
    let ng1 = if s == 0 { 0 } else { s.div_ceil(g) };
    let p = ng1 * t;
    let nsq = s + p;
    let ct = 4 * nsq;
    let (ct_triple, c, p2, nsq2, m) = if ct <= CT_TRIPLE_MAX {
        ...
        let c = ct.div_ceil(blk);
        let ng2 = c.div_ceil(g);
        let p2 = ng2 * t;
        let nsq2 = c + p2;
    ...
    let nslots = nsq + nsq2;
    let site = HDR + msize;
    let total = 3 * site + nslots * blk;
    let mid = if nslots == 0 {
        0
    } else {
        let want = total / 2;
        let j = if want > site { (want - site + blk / 2) / blk } else { 0 };
        j.clamp(1, nslots)
    };
    Geom { len, blk, gmax, g, t, s, p, nsq, ct, ct_triple, c, p2, nsq2, m, msize, nslots, mid, hash, ex, total }
}
/// the 4 KB-scratch claim for an existing container's geometry
pub fn scratch_guaranteed(g: &Geom) -> bool {
    g.g != 0 && guaranteed_st(g.s, g.g, g.t, g.blk)
}
...
pub fn offsets(g: &Geom) -> Off {
    ...
    let h1 = site + g.mid * g.blk;
    let m2 = 2 * site + g.nslots * g.blk;
...
pub fn slot_off(g: &Geom, j: usize) -> usize {
    let site = HDR + g.msize;
    site + j * g.blk + if j >= g.mid { site } else { 0 }
}
...
fn residue(sq: &[u8], m: u32) -> u32 {
    // squares arrive full tier width (padded at build); length IS the tier
    let mut acc: u32 = 0;
    for &b in sq {
        acc = (acc * 256 + b as u32) % m;
    }
    acc
}
pub struct Syn {
    pow: Vec<u32>, // nbits entries: 8 * blk of the tier this table serves
    map: Vec<i32>,
    m: u32,
}
pub fn syn(m: u32, nbits: usize) -> Syn {
    let mut pow = vec![0u32; nbits];
    let mut x = 1u32;
    for w in 0..nbits {
        pow[nbits - 1 - w] = x;
        x = (x * 2) % m;
    ...
    for (i, &pwi) in pow.iter().enumerate() {
        for (d, neg) in [(pwi % m, 0i32), ((m - pwi) % m, 1i32)] {
...
// the syndrome tables are pure functions of (modulus, tier): built once per
// tier, kept for the process (v7 rebuilt them on every dearmor call; the
// audit's boundary maps convicted that as the dominant cost)
static SYNS: [OnceLock<(Syn, Syn)>; 3] = [OnceLock::new(), OnceLock::new(), OnceLock::new()];
pub fn syn_tier(blk: usize) -> &'static (Syn, Syn) {
    let ti = tier_index(blk);
    SYNS[ti].get_or_init(|| {
        let (b, p, q) = TIERS[ti];
        (syn(p, 8 * b), syn(q, 8 * b))
    })
}
pub fn syn_distinct(m: u32, nbits: usize) -> usize {
    // audit(b): how many of the 2*nbits signed syndromes are distinct entries
    let t = syn(m, nbits);
...
pub fn syndrome_pair(blk: usize, i: u16, d: i8) -> (u32, u32) {
    let (sp, sq) = syn_tier(blk);
    (syndrome_of(sp, i, d), syndrome_of(sq, i, d))
}
...
fn bit_repair(sq: &mut [u8], rp: u32, rq: u32, sp_tab: &Syn, sq_tab: &Syn, doubles: bool) -> Option<usize> {
    let (mp, mq) = (sp_tab.m, sq_tab.m);
    let sp = (residue(sq, mp) + mp - rp) % mp;
    let s2 = (residue(sq, mq) + mq - rq) % mq;
    if sp == 0 && s2 == 0 {
        return Some(0);
    }
    ...
    if doubles {
        ...
        for i1 in 0..(sq.len() * 8).min(sp_tab.pow.len()) as u16 {
            let d1: i8 = if get_bit(sq, i1 as u64) == 1 { 1 } else { -1 };
            let rem = (sp + mp - syndrome_of(sp_tab, i1, d1)) % mp;
            if rem == 0 {
                continue;
            }
            if let Some((i2, d2)) = lookup(sp_tab, rem) {
                ...
                if (syndrome_of(sq_tab, i1, d1) + syndrome_of(sq_tab, i2, d2)) % mq != s2 {
                    continue;
                }
...
fn write_header(g: &Geom, out: &mut [u8]) {
    out[0..4].copy_from_slice(b"EG11");
    out[4] = 5;
    out[5] = g.g as u8;
    let (_, tp, tq) = TIERS[tier_index(g.blk)];
    out[6..8].copy_from_slice(&(tp as u16).to_le_bytes());
    out[8..10].copy_from_slice(&(tq as u16).to_le_bytes());
    ...
    out[53] = tier_index(g.blk) as u8; // v5: the square tier, FNV-covered
    for b in out[54..60].iter_mut() {
        *b = 0;
    }
...
fn parse_header(b: &[u8]) -> Option<(Geom, bool)> {
    // v11 writes EG11 v5; EG10 v4, EGG9 v3 and EGG8 v2 containers (same layout)
    ...
    let v10_compat = &b[0..4] == b"EG10" && b[4] == 4;
    if b.len() < HDR || (!v8_compat && !v9_compat && !v10_compat && (&b[0..4] != b"EG11" || b[4] != 5)) {
        return None;
    }
    let v5 = &b[0..4] == b"EG11";
    let (blk, gmax) = if v5 {
    ...
    let (_, tp, tq) = TIERS[tier_index(blk)];
    if u16::from_le_bytes([b[6], b[7]]) as u32 != tp
        || u16::from_le_bytes([b[8], b[9]]) as u32 != tq
    {
        return None;
    }
    ...
    let g = geom(len, blk, gmax, grp, t, hash, ex);
    if g.s != s {
...
/// squares live in ONE flat buffer, blk bytes each (the tier decides blk)
fn fill_square(dst: &mut [u8], src: &[u8], i: usize, blk: usize) {
    dst.fill(0);
    let base = i * blk;
    if base < src.len() {
        let n = (src.len() - base).min(blk);
        dst[..n].copy_from_slice(&src[base..base + n]);
    }
}
fn shield_squares(src: &[u8], s: usize, g: usize, t: usize, gf: &Gf, blk: usize, gmax: usize) -> Vec<u8> {
    let ng = if s == 0 { 0 } else { s.div_ceil(g) };
    let p = ng * t;
    let mut sq = vec![0u8; (s + p) * blk];
    for i in 0..s {
        let mut tmp = vec![0u8; blk];
        fill_square(&mut tmp, src, i, blk);
        sq[i * blk..(i + 1) * blk].copy_from_slice(&tmp);
    }
    for pg in 0..ng {
        let lo = pg * g;
        ...
        for pt in 0..t {
            ...
            let mut acc = vec![0u8; blk];
            for (i, &wi) in w.iter().enumerate() {
                if wi == 0 {
                    continue;
                }
                let srcsq = &sq[(lo + i) * blk..(lo + i + 1) * blk];
                for (a, &sb) in acc.iter_mut().zip(srcsq.iter()) {
                    *a ^= gf.mul(wi, sb);
                }
            }
            sq[(s + pg * t + pt) * blk..(s + pg * t + pt + 1) * blk].copy_from_slice(&acc);
        }
    }
    sq
}
fn checks_of(sq: &[u8], blk: usize, mp: u32, mq: u32) -> Vec<u8> {
    let n = sq.len() / blk;
    let mut ct = vec![0u8; 4 * n];
    for (i, s) in sq.chunks_exact(blk).enumerate() {
        let rp = residue(s, mp);
        let rq = residue(s, mq);
...
// ---------- armor (encode) ----------
pub fn armor(inner: &[u8], blk: usize, grp: usize, tpar: usize, ex: Extras) -> Vec<u8> {
    let g = geom(inner.len(), blk, GMAX5, grp, if grp == 0 { 0 } else { tpar }, fnv64(inner), ex);
    let off = offsets(&g);
    ...
    let gf = gf_new();
    let (_, tp, tq) = TIERS[tier_index(g.blk)];
    let sq1 = shield_squares(inner, g.s, g.g, g.t, &gf, g.blk, g.gmax);
    let ct1 = checks_of(&sq1, g.blk, tp, tq);
    let (meta_body, sq2) = if g.ct_triple {
        (ct1, Vec::new())
    } else {
        let sq2 = shield_squares(&ct1, g.c, g.g, g.t, &gf, g.blk, g.gmax);
        (checks_of(&sq2, g.blk, tp, tq), sq2)
    };
    ...
    for (j, sl) in slot_order(&g).iter().enumerate() {
        let i = sl.idx as usize;
        let sq = if sl.level == 0 { &sq1[i * g.blk..(i + 1) * g.blk] } else { &sq2[i * g.blk..(i + 1) * g.blk] };
        let o = slot_off(&g, j);
        out[o..o + g.blk].copy_from_slice(sq);
    }
    out
}
...
pub fn dearmor(cont_in: &[u8], wounds_in: &[(usize, usize)], doubles: bool) -> Result<DearmorOut, String> {
    ...
    let seed = select_header(cont_in, &[0, cont_in.len().saturating_sub(HDR)]).or_else(|e| {
        ...
        while i + HDR <= cont_in.len() {
            if &cont_in[i..i + 4] == b"EG11"
                || &cont_in[i..i + 4] == b"EG10"
    ...
    let (sp_tab, sq_tab) = syn_tier(g.blk);
    let gf = gf_new();
    ...
    let blk = g.blk;
    let gather = |level: u8, n: usize| -> (Vec<u8>, Vec<bool>) {
        let mut sq = vec![0u8; n * blk];
        ...
        for (j, sl) in order.iter().enumerate() {
            ...
            let a = slot_off(&g, j);
            let i = sl.idx as usize;
            sq[i * blk..(i + 1) * blk].copy_from_slice(&cont[a..a + blk]);
            if in_wound(a, a + blk) {
                dead[i] = true;
            }
        }
    ...
    let repair = |sq: &mut Vec<u8>,
        ...
        for i in 0..nsq {
            ...
            if dead[i] || mode == Repair::ParityOnly {
                ...
                let ok = residue(sqi, sp_tab.m) == rp && residue(sqi, sq_tab.m) == rq;
                if ok { t.clean += 1 } else { bad[i] = true }
            } else {
                match bit_repair(sqi, rp, rq, sp_tab, sq_tab, mode == Repair::Full) {
                    Some(0) => t.clean += 1,
                    Some(1) => t.bitfixed += 1,
        ...
        let ng = if n_data == 0 { 0 } else { n_data.div_ceil(g.g) };
        for pg in 0..ng {
            let lo = pg * g.g;
            let m = ((pg + 1) * g.g).min(n_data) - lo;
            let members: Vec<(u8, usize)> = (0..m)
                .map(|i| (i as u8, lo + i))
                .chain((0..g.t).map(|pt| ((g.gmax + pt) as u8, n_data + pg * g.t + pt)))
                .collect();
            ...
            for &k in &bads {
                ...
                for (wi, &bk) in w.iter().zip(base.iter()) {
                    ...
                    let si = members[bk].1;
                    let src = &sq[si * blk..(si + 1) * blk];
                    for (a, &sb) in acc.iter_mut().zip(src.iter()) {
                        *a ^= gf.mul(*wi, sb);
                    }
                }
                let (rp, rq) = checks(idx);
                if residue(&acc, sp_tab.m) == rp && residue(&acc, sq_tab.m) == rq {
            ...
            if ok_all {
                for (idx, acc) in rebuilt {
                    sq[idx * blk..(idx + 1) * blk].copy_from_slice(&acc);
                    bad[idx] = false;
                    t.rebuilt += 1;
                }
            } else {
    ...
    let run = |mode: Repair| -> (Vec<u8>, Tally, String) {
        ...
        let (ct, ct_report) = if g.ct_triple {
            ...
            for (i, sqi) in sq2.chunks_exact(blk).enumerate().take(g.c) {
                let base = i * blk;
                let n = (g.ct - base).min(blk);
                ct[base..base + n].copy_from_slice(&sqi[..n]);
            }
            let rep = format!(
        ...
        for (i, sqi) in sq1.chunks_exact(blk).enumerate().take(g.s) {
            let base = i * blk;
            let n = (g.len - base).min(blk);
            data[base..base + n].copy_from_slice(&sqi[..n]);
        }
        (data, t, ct_report)
    };
