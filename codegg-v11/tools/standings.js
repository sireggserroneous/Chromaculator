...
const EGG11 = path.join(here, "..", "target", "release", "eggv11" + (process.platform === "win32" ? ".exe" : ""));
...
function attempt(name, damaged, at, len, kind, src){
  try {
    ...
    else if(name === "egg7" || name === "egg8" || name === "egg9" || name === "egg10" || name === "egg11"){
      const exe = name === "egg11" ? EGG11 : name === "egg10" ? EGG10 : name === "egg9" ? EGG9 : name === "egg8" ? EGG8 : EGG7;
...
const NAMES = ["gzip", "zstd", "brotli", "xz", "egg6", "egg6+zstd", "egg7", "egg8", "egg9", "egg10", "egg11"];
...
for(const file of files){
  ...
  total++;
  if(verdicts.egg11.ok){
    if(verdicts.egg11.size < art._gzStar) barGz++;
    if(verdicts.egg11.size < verdicts["egg6+zstd"].size) barHybrid++;
    if(verdicts.egg11.size < verdicts.xz.size) barXz++;
    if(verdicts.egg11.size <= Math.min(verdicts.egg8.size, verdicts.egg9.size, verdicts.egg10.size)) barMin++;
  }
}
console.log("-".repeat(169));
console.log("podium: " + Object.entries(tally).sort((a, b) => b[1] - a[1])
  .map(([n, c]) => `${n} x${c}`).join(", "));
console.log(`victory ledger (egg11, armor ON, all injuries EXACT, of ${total} files):`);
console.log(`  vs naked xz -9               : ${barXz}/${total}   (v11 bars: home >=10 of 12, big >=5 of 8)`);
console.log(`  vs strongest gzip -9         : ${barGz}/${total}`);
console.log(`  vs egg6+zstd hybrid          : ${barHybrid}/${total}`);
console.log(`  <= min(egg8,egg9,egg10)      : ${barMin}/${total}  (v11 bar: ALL rows)`);
