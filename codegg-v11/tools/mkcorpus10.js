#!/usr/bin/env node
// mkcorpus10.js -- deterministic corpus for the v8 tournament.
...
// program.exe (the then-current eggv10 build; a real PE either way).
...
const root = path.join(here, '..');          // codegg-v10/
...
// 4. program.exe -- the then-current eggv10 build, a real PE
fs.copyFileSync(path.join(root, 'target', 'release', 'eggv10.exe'), path.join(out, 'program.exe'));
