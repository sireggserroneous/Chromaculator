/* the M9 big-arena countersign, in lanes: eggv11 transmute + restore +
 ...
 * official run (deterministic; those binaries are frozen) -- only the egg11
...
async function doFile(f){
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'cs11-'));
  const art = path.join(tmp, f + '.egg11'), out = path.join(tmp, f + '.out');
  try {
