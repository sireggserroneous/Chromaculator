...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'egg11drill-'));
...
function drill(name, contBuf, orig, wounds, expect) {
  const p = path.join(tmp, 'drill.egg11');
...
for (const { f, label } of cases) {
  ...
  const cont = path.join(tmp, f + '.egg11');
  transmute(src, cont);
  const g = info(cont);
...
// 3-bit storms x300 on the mid artifact
{
  ...
  for (let i = 0; i < 300; i++) {
    const p = path.join(tmp, 'storm.egg11');
...
// no-armor: pristine exact; any wound must fail honestly (hash gate)
{
  ...
  const src = path.join(corpus, f);
  const orig = fs.readFileSync(src);
  const cont = path.join(tmp, f + '.noarmor.egg11');
