...
fn bit_repair(sq: &mut [u8], rp: u32, rq: u32, sp_tab: &Syn, sq_tab: &Syn, doubles: bool) -> Option<usize> {
    ...
    if doubles {
        ...
        for i1 in 0..(sq.len() * 8).min(1024) as u16 {
...
fn parse_header(b: &[u8]) -> Option<Geom> {
    if b.len() < HDR || &b[0..4] != b"EGG5" || b[4] != 4 || b[5] == 0 {
        return None;
    }
    let t = b[30] as usize;
...
fn checks_of(sq: &[[u8; BLOCK]]) -> Vec<u8> {
    let mut ct = vec![0u8; 3 * sq.len()];
    for (i, s) in sq.iter().enumerate() {
        let rp = residue(s, P);
        let rq = residue(s, Q);
        ct[3 * i] = (rp & 0xff) as u8;
        ct[3 * i + 1] = (((rp >> 8) & 0x0f) | ((rq & 0x0f) << 4)) as u8;
        ct[3 * i + 2] = ((rq >> 4) & 0xff) as u8;
    }
    ct
}
...
fn decode(cont_in: &[u8], wounds_in: &[(usize, usize)], doubles: bool) -> Result<DecodeOut, String> {
    ...
    let meta_checks = move |i: usize| -> (u32, u32) {
        let o = 3 * i;
        let rp = (meta[o] as u32) | (((meta[o + 1] & 0x0f) as u32) << 8);
        let rq = (((meta[o + 1] >> 4) & 0x0f) as u32) | ((meta[o + 2] as u32) << 4);
        (rp, rq)
    };
    ...
    let run = |mode: Repair| -> (Vec<u8>, Tally, String) {
        ...
        let checks = move |i: usize| -> (u32, u32) {
            let o = 3 * i;
            let rp = (ct[o] as u32) | (((ct[o + 1] & 0x0f) as u32) << 8);
            let rq = (((ct[o + 1] >> 4) & 0x0f) as u32) | ((ct[o + 2] as u32) << 4);
            (rp, rq)
        };
