...
//!   * `v5_figures` -- **C1**, and it is the gate this round rests on. v6
...
fn read_record(slug: &str) -> Result<String, String> {
    let p = repo_root().join("eggSo-v5").join(format!("measured-{slug}.json"));
    std::fs::read_to_string(&p).map_err(|e| format!("{}: {e}", p.display()))
}

/// C1. v6 copies v5's `fold`, `code`, `seam`, `optimum`, `json` and `pin`
/// rather than depending on them, because each round here is a frozen record
/// and a path dependency would let v6's numbers drift when v5 changes. The
/// price of copying is that a copy can drift SILENTLY, so this recomputes
/// v5's headline figures with v6's copies and compares them against v5's
/// committed `measured-*.json`:
///
///   * `diag3` at `341/342/341`, separation `0.6673`, 48 check bits;
///   * the linear theorem's solution counts by residue, `0 / 0 / 2`;
///   * **v5's same-class-double collapse at `n = 512`, 70 of 120** -- the one
///     figure this whole round exists to move, so it is pinned before any
///     cap is touched.
///
/// No node. If a record file is missing the pin reports SKIPPED loudly.
pub fn v5_figures() -> PinResult {
    const NAME: &str = "the copy vs v5's record";
    let mut checked = 0usize;
    let mut mismatches = 0usize;
    let mut check = |cond: bool| {
        checked += 1;
    ...
    // -- diag3's classes and separation, from measured-arms.json ------------
    let arms = match read_record("arms") {
        Ok(t) => t,
    ...
    // -- the linear theorem, from measured-optimum.json --------------------
    let opt = match read_record("optimum") {
        Ok(t) => t,
        Err(e) => return PinResult::skipped(NAME, e),
    };
    let mut s = Scan::new(&opt);
    check(s.seek("\"theorem\""));
    for (key, want) in [
        ("\"solutionsAtNmod3Zero\"", 0usize),
        ("\"solutionsAtNmod3One\"", 0),
        ("\"solutionsAtNmod3Two\"", 2),
    ] {
        check(s.after(key) == Some(want as f64));
    }
    // and re-derived here rather than read
    check(crate::optimum::linear_solutions(30).is_empty());
    check(crate::optimum::linear_solutions(31).is_empty());
    check(crate::optimum::linear_solutions(32).len() == 2);

    // -- the figure this round exists to move ------------------------------
    let sizes_rec = match read_record("sizes") {
        Ok(t) => t,
        Err(e) => return PinResult::skipped(NAME, e),
    };
    let mut s = Scan::new(&sizes_rec);
    check(s.seek("\"price\""));
    let mut theirs_512: Option<f64> = None;
    // walk the price rows to the n = 512 entry
    for _ in 0..24 {
        if !s.seek("\"n\"") {
            break;
        }
        let n = s.num();
        if n == Some(512.0) {
            theirs_512 = s.after("\"sameClassDoubleCorrected\"");
            break;
        }
    }
    check(theirs_512 == Some(70.0));
    let big = crate::seam::seams().into_iter().find(|x| x.name == "diag3").unwrap().code(512, true);
    let mine = crate::seam::run_channel(&big, crate::seam::Channel::TwoSameClass, 120, 7);
    match mine {
        Some(t) => {
            check(t.corrected == 70);
            check(t.wrong == 0);
        }
        None => check(false),
    }
...
mod tests {
    ...
    fn a_perturbed_record_reads_back_perturbed() {
        let Ok(real) = read_record("arms") else {
            return; // v5's record is not on disk; v5_figures reports SKIPPED
        };
        let mut s = Scan::new(&real);
        assert!(s.seek("\"arm\": \"diag3\""));
        assert!(s.seek("\"classes\""));
        assert_eq!(s.num(), Some(341.0), "v5 recorded diag3 at 341/342/341");
    ...
    #[test]
    fn the_v5_figures_pin_checks_something_and_agrees() {
        let r = v5_figures();
