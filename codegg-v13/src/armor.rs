...
fn parse_header(b: &[u8]) -> Option<(Geom, bool)> {
    // v13 writes EG13 v7; EG12 v6 is READ by this same armor v4 path
    // (the format did not move -- only the name did), so every .egg12
    // container restores here, byte for byte, and armor11.rs still owns
    // .egg11 and older.
    if b.len() < HDR {
        return None;
    }
    if !((&b[0..4] == MAGIC && b[4] == FORMAT_VERSION) || (&b[0..4] == MAGIC_V12 && b[4] == 6)) {
        return None;
    }
...
pub fn dearmor_with(cont_in: &[u8], wounds_in: &[(usize, usize)], trust_residues: bool) -> Result<DearmorOut, String> {
    ...
    let seed = select_header(cont_in, &[0, cont_in.len().saturating_sub(HDR)]).or_else(|e| {
        ...
        while i + HDR <= cont_in.len() {
            if &cont_in[i..i + 4] == MAGIC || &cont_in[i..i + 4] == MAGIC_V12 {
