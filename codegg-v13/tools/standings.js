...
const EGG13 = path.join(here, "..", "target", "release", "eggv13" + (process.platform === "win32" ? ".exe" : ""));
...
function buildArtifacts(file, src){
  ...
  cp.execSync(`"${EGG13}" transmute "${file}" -o "${SP}/s.egg13"`, {stdio: "pipe"});
  art.egg13 = fs.readFileSync(`${SP}/s.egg13`);
...
function attempt(name, damaged, at, len, kind, src){
  try {
    ...
    if(name === "gzip") out = zlib.gunzipSync(damaged);
      ...
      const exe = name === "egg13" ? EGG13 : name === "egg12" ? EGG12 : name === "egg11" ? EGG11 : name === "egg10" ? EGG10 : name === "egg9" ? EGG9 : name === "egg8" ? EGG8 : EGG7;
...
for(const file of files){
  ...
  total++;
  if(verdicts.egg13.ok){
    if(verdicts.egg13.size < art._gzStar) barGz++;
    if(verdicts.egg13.size < verdicts["egg6+zstd"].size) barHybrid++;
    if(verdicts.egg13.size < verdicts.xz.size) barXz++;
    if(verdicts.egg13.size <= Math.min(verdicts.egg8.size, verdicts.egg9.size, verdicts.egg10.size, verdicts.egg11.size, verdicts.egg12.size)) barMin++;
  }
}
console.log("-".repeat(180));
