...
const EGG9 = path.join(here, "..", "target", "release", "eggv13" + (process.platform === "win32" ? ".exe" : ""));
...
for(const file of process.argv.slice(2)){
  ...
  cp.execSync(`"${EGG9}" transmute "${file}" -o "${SP}/w.egg13"`, {stdio: "pipe"});
  const art = fs.readFileSync(`${SP}/w.egg13`);
