...
const EXE = process.env.EGG_EXE || path.join(root, 'target', 'release', 'eggv13.exe'); // EGG_EXE: a copied build (the exe lock)
...
async function doFile(dir, f){
  ...
  const art = path.join(tmp, f + '.egg13');
  ...
  try {
    ...
    for(const kind of ['flip', 'scratch', 'trunc']){
      ...
      const dp = path.join(tmp, 'd.egg13'), op = path.join(tmp, 'd.out');
