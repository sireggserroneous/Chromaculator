/* the M9 big-arena countersign, in lanes: eggv13 transmute + restore +
 ...
 * official run (deterministic; those binaries are frozen) -- only the egg13
 * column is new, and this signs it. */
const fs = require('fs'), path = require('path'), os = require('os');
const { execFile, execFileSync } = require('child_process');
const root = path.join(path.dirname(__filename), '..');
const EXE = path.join(root, 'target', 'release', 'eggv13.exe');
...
async function doFile(f){
  ...
  const art = path.join(tmp, f + '.egg13'), out = path.join(tmp, f + '.out');
  try {
