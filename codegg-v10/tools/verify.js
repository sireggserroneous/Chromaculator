...
const EGG9 = path.join(here, "..", "target", "release", "eggv10" + (process.platform === "win32" ? ".exe" : ""));
...
for(const file of process.argv.slice(2)){
  ...
  cp.execSync(`"${EGG9}" transmute "${file}" -o "${SP}/w.egg10"`, {stdio: "pipe"});
  const art = fs.readFileSync(`${SP}/w.egg10`);
  const short = file.split(/[\\/]/).pop();
  console.log(`${short}  (egg10, ${(100 * art.length / src.length).toFixed(1)}% of original)`);
