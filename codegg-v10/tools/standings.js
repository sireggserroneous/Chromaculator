/* node codegg-v10/tools/standings.js <file>... -- the tournament, v10 row added.
 *
...
const EGG10 = path.join(here, "..", "target", "release", "eggv10" + (process.platform === "win32" ? ".exe" : ""));
...
function attempt(name, damaged, at, len, kind, src){
  try {
    ...
    else if(name === "egg7" || name === "egg8" || name === "egg9" || name === "egg10"){
      const exe = name === "egg10" ? EGG10 : name === "egg9" ? EGG9 : name === "egg8" ? EGG8 : EGG7;
...
const NAMES = ["gzip", "zstd", "brotli", "xz", "egg6", "egg6+zstd", "egg7", "egg8", "egg9", "egg10"];
...
for(const file of files){
  ...
  total++;
  if(verdicts.egg10.ok){
    if(verdicts.egg10.size < art._gzStar) barGz++;
    if(verdicts.egg10.size < verdicts["egg6+zstd"].size) barHybrid++;
    if(verdicts.egg10.size < verdicts.xz.size) barXz++;
  }
}
console.log("-".repeat(158));
console.log("podium: " + Object.entries(tally).sort((a, b) => b[1] - a[1])
  .map(([n, c]) => `${n} x${c}`).join(", "));
console.log(`victory ledger (egg10, armor ON, all injuries EXACT, of ${total} files):`);
console.log(`  THE BAR vs xz -9             : ${barXz}/${total}  (needs >=8 of 12) ${barXz >= 8 ? "-- MET" : "-- NOT met"}`);
console.log(`  held: vs strongest gzip -9   : ${barGz}/${total}  (v8 bar >=10) ${barGz >= 10 ? "-- MET" : "-- NOT met"}`);
console.log(`  held: vs egg6+zstd hybrid    : ${barHybrid}/${total}  (v8 bar >=6) ${barHybrid >= 6 ? "-- MET" : "-- NOT met"}`);
