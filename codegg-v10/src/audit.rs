...
use crate::armor::{
    armor, dearmor, fnv64, geom, guaranteed_st, offsets, rib_policy, slot_off, slot_order,
    syn_distinct, syndrome_pair, Extras, Geom, BLOCK, DEAD9, HDR, P, Q,
};
...
/// slide every DEAD9-slot window over the real merged order; return the most
/// members any single group loses to one window, and the windows examined.
...
pub fn run_gate() -> (bool, u64) {
    ...
    for s in 1..=2000usize {
        ...
        let g = geom(len, rib.g, rib.t, 0, dummy_ex());
    ...
    let grids: [(usize, usize); 10] =
        [(4, 2), (4, 3), (5, 2), (9, 2), (11, 2), (13, 3), (31, 2), (63, 2), (126, 2), (126, 3)];
    let mut grid_claims = 0u64;
    for s in 1..=2000usize {
        for &(gg, tt) in &grids {
            let g = geom(s * BLOCK, gg, tt, 0, dummy_ex());
            let (worst, w) = worst_window(&g);
            checks += w;
            if guaranteed_st(s, gg, tt) {
                grid_claims += 1;
                if worst > tt {
                    ok = false;
                    println!(
                        "  FAIL (a) predicate s={} G{} T{}: claimed but window kills {}",
                        s, gg, tt, worst
                    );
                }
            }
    ...
    println!(
        "  (a) predicate, adversarial grids (10 rib shapes x 2000 sizes): {} claims, none over-claimed",
        grid_claims
    );
    ...
    for s in 12..=2000usize {
        let gg = ((s + 4) / 5).clamp(4, 126);
        let g = geom(s * BLOCK, gg, 2, 0, dummy_ex());
        let (worst, w) = worst_window(&g);
        checks += w;
    ...
    for i in 0..200 {
        ...
        let g = geom(s * BLOCK, rib.g, rib.t, 0, dummy_ex());
        let (worst, w) = worst_window(&g);
        checks += w;
        sampled += 1;
    ...
    // ---------- (b) residue injectivity ----------
    for m in [P, Q] {
        let d = syn_distinct(m);
        checks += 8192;
        if d != 8192 {
            ok = false;
        }
        println!(
            "  (b) modulus {}: {}/8192 signed syndromes +-2^k (k<4096) distinct{}",
            m,
            d,
            if d == 8192 { "" } else { " -- FAIL" }
        );
    ...
    let mut ambig = 0u64;
    let nsamp = 200_000u64;
    let mut st2 = 0xACEu64;
    for _ in 0..nsamp {
        let i1 = (xorshift(&mut st2) % 4096) as u16;
        let mut i2 = (xorshift(&mut st2) % 4096) as u16;
        if i2 == i1 {
            i2 = (i2 + 1) % 4096;
        }
        let d1: i8 = if xorshift(&mut st2) & 1 == 1 { 1 } else { -1 };
        let d2: i8 = if xorshift(&mut st2) & 1 == 1 { 1 } else { -1 };
        let (p1, q1) = syndrome_pair(i1, d1);
        let (p2, q2) = syndrome_pair(i2, d2);
    ...
    checks += nsamp;
    println!(
        "  (b) double-error ambiguity, {} sampled pairs: {} dual-modulus collisions ({:.2e}/pair) -- why the ladder's last rung trusts parity only",
        nsamp,
        ambig,
...
fn make_artifact(inner_len: usize, seed: u64) -> (Vec<u8>, Vec<u8>, Geom) {
    ...
    let cont = armor(&inner, rib.g, rib.t, ex);
    let g = geom(inner.len(), rib.g, rib.t, 0, ex);
    (inner, cont, g)
}
...
fn boundary_maps() -> (bool, u64) {
    ...
    for &inner_len in &[8 * 1024usize, 30 * 1024, 100 * 1024, 500 * 1024, 4 * 1024 * 1024] {
        ...
        let theory = klo.saturating_sub(1) * BLOCK + 1; // k dead slots span >= (k-1)*B+1 bytes
        let anchors: Vec<(&str, usize, bool)> = {
            let first_l2 = order.iter().position(|s| s.level == 1).map(|j| slot_off(&g, j));
            let mut v = vec![
                ("head", 0usize, true),
                ("payload", off.slot_base + (g.mid / 2) * BLOCK, true),
...
fn fuzz() -> (bool, u64) {
    let mut ok = true;
    let arts: Vec<(Vec<u8>, Vec<u8>, Geom)> = [8 * 1024usize, 30 * 1024, 100 * 1024, 500 * 1024]
        .iter()
...
fn adversarial() -> (bool, u64) {
    ...
    for &inner_len in &[30 * 1024usize, 500 * 1024] {
        ...
        let derive = |wounds: &[(usize, usize)]| -> Verdict {
            ...
            for (j, sl) in order.iter().enumerate() {
                let a = slot_off(&g, j);
                if wounds.iter().any(|&(w, l)| w < a + BLOCK && w + l > a) {
        ...
        run_case("two disjoint 1-slot scratches", &[(a1, BLOCK), (a2, BLOCK)], &mut ok, &mut checks);
        // T+1 members of payload group 0 via slot arithmetic (must refuse)
        let mut w = Vec::new();
        for k in 0..=g.t {
            w.push((slot_off(&g, k * ngt), BLOCK));
        }
        ...
        if !g.ct_triple {
            ...
            for k in 0..g.t {
                w.push((slot_off(&g, k * ngt), BLOCK));
                w.push((slot_off(&g, ng1 + k * ngt), BLOCK));
            }
        ...
        run_case(
            ...
            &[(off.h0, site), (cont.len() - site, site), (slot_off(&g, 3), BLOCK)],
            &mut ok,
            &mut checks,
        );
        // region straddle: scratch across the mid site boundary
        run_case("mid-site straddle", &[(off.h1.saturating_sub(BLOCK / 2), site + BLOCK)], &mut ok, &mut checks);
        // scratch + 8-bit storm on top: never silent (verdict may be either)
        {
            let at = slot_off(&g, 1);
            let mut hurt = scratch(&cont, at, BLOCK, 0xF00D);
