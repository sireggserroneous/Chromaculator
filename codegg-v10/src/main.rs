...
const MODEL_MIX10: u8 = 8; // one dyadic point, v10 model (the Keeper)
const MODEL_CM10: u8 = 9; // one dyadic point, literal-only v10
...
fn encode_best(src: &[u8], toks: &[token::Tok]) -> (Vec<u8>, u8) {
    let (v8, v10, cm) = std::thread::scope(|s| {
        let h10 = s.spawn(|| dyadic::encode10(src, toks));
        let hc = s.spawn(|| dyadic::encode_cm10(src));
        (encode_best_v8(src, toks), h10.join().expect("trial"), hc.join().expect("trial"))
    });
    ...
    let v10 = if src.len() >= 512 * 1024 {
        let p8 = replay_price8(src.len(), toks, v10.len());
        let toks2 = token::tokenize_priced(src, p8);
        let vb = dyadic::encode10(src, &toks2);
        if vb.len() < v10.len() { vb } else { v10 }
    } else {
        v10
    };

    // contingent MIX9 fallback: if BOTH v10 big arms lose to the frozen v8
    // pick, that is a loud regression signal -- v9's model gets one shot
    // before the verdict (the measured vs-v9 gate makes this ~never fire)
    if v10.len() > v8.0.len() && cm.len() > v8.0.len() {
        let v9 = dyadic::encode9(src, toks);
        if v9.len() < v8.0.len() {
            return (v9, MODEL_MIX9);
        }
    }
    if cm.len() <= v10.len() && cm.len() <= v8.0.len() {
        (cm, MODEL_CM10)
    } else if v10.len() <= v8.0.len() {
        (v10, MODEL_MIX10)
    } else {
        v8
    }
...
fn filtered_transmute(src: &[u8], forced: Option<(u8, u32)>) -> (Vec<u8>, u8, u8, u32) {
    ...
    let (mix10_plain, cm10_plain) = std::thread::scope(|s| {
        let hc = s.spawn(|| dyadic::encode_cm10(src));
        let mut p = dyadic::encode10(src, &toks_plain);
        if src.len() >= 512 * 1024 {
            let p8 = replay_price8(src.len(), &toks_plain, p.len());
            let toks2 = token::tokenize_priced(src, p8);
            let pb = dyadic::encode10(src, &toks2);
            if pb.len() < p.len() {
                p = pb;
            }
        }
        (p, hc.join().expect("cm10 trial"))
    });
    let filt_big: Option<(Vec<u8>, Vec<u8>)> = best.map(|i| {
        let f = &filtered_srcs[i];
        std::thread::scope(|s| {
            let hc = s.spawn(|| dyadic::encode_cm10(f));
            let toks = token::tokenize(f);
            let mut m = dyadic::encode10(f, &toks);
            if f.len() >= 512 * 1024 {
                let p8 = replay_price8(f.len(), &toks, m.len());
                let toks2 = token::tokenize_priced(f, p8);
                let mb = dyadic::encode10(f, &toks2);
                if mb.len() < m.len() {
                    m = mb;
                }
            }
            ...
            if cands[i].id == filter::FILTER_W16 || cands[i].id == filter::FILTER_W16O2 {
                ...
                let mc = dyadic::encode10(f, &toks8);
                if mc.len() < m.len() {
                    m = mc;
                }
            }
            (m, hc.join().expect("cm10 trial"))
        })
    });
    ...
    if mix10_plain.len() <= best_plain.0 {
        best_plain = (mix10_plain.len(), &mix10_plain, MODEL_MIX10);
    }
    if cm10_plain.len() <= best_plain.0 {
        best_plain = (cm10_plain.len(), &cm10_plain, MODEL_CM10);
    }
    let best_filt: Option<(usize, &Vec<u8>, u8, usize)> = best.map(|i| {
        let t8 = &trials8[i];
        let mut bf: (usize, &Vec<u8>, u8) = (t8.0.len(), &t8.0, t8.1);
        if let Some((m10, c10)) = &filt_big {
            if m10.len() <= bf.0 {
                bf = (m10.len(), m10, MODEL_MIX10);
            }
            if c10.len() <= bf.0 {
                bf = (c10.len(), c10, MODEL_CM10);
            }
        }
        (bf.0, bf.1, bf.2, i)
    });
...
fn main() -> ExitCode {
    ...
    let usage = || {
        eprintln!("usage: eggv10 transmute <file> [-o out.egg10] [--no-armor] [--group N] [--parity T]");
        eprintln!("       eggv10 restore <file.egg10 | .egg9 | .egg8> [-o out] [--wound start:len] [--no-doubles]");
        eprintln!("       eggv10 scratch <file> [--len 4096] [--at payload|checks|head|end|<off>]");
        eprintln!("       eggv10 audit [--full]   -- the geometry audit, counts printed");
        ExitCode::from(2)
    };
    ...
    match cmd {
        "transmute" => {
            ...
            let (inner, model, fid, fparam) = if model_req == MODEL_DYADIC && !has("--stats") {
                ...
                let out9 = dyadic::encode10(&fsrc, &toks);
                let outc = dyadic::encode_cm10(&fsrc);
            ...
            let dst = get("-o").unwrap_or(format!("{}.egg10", path));
