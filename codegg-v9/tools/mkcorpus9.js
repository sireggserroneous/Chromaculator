#!/usr/bin/env node
// mkcorpus9.js -- deterministic corpus for the v8 tournament.
...
// program.exe (the then-current eggv9 build; a real PE either way).
...
const root = path.join(here, '..');          // codegg-v9/
...
// 4. program.exe -- the then-current eggv9 build, a real PE
fs.copyFileSync(path.join(root, 'target', 'release', 'eggv9.exe'), path.join(out, 'program.exe'));
