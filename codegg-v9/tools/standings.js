/* node codegg-v9/tools/standings.js <file>... -- the tournament, v9 row added.
 *
...
function attempt(name, damaged, at, len, kind, src){
  try {
    ...
    else if(name === "egg7" || name === "egg8" || name === "egg9"){
      const exe = name === "egg9" ? EGG9 : name === "egg8" ? EGG8 : EGG7;
...
const NAMES = ["gzip", "zstd", "brotli", "xz", "egg6", "egg6+zstd", "egg7", "egg8", "egg9"];
...
console.log("file                    orig B      gzip    zstd  brotli      xz    egg6 e6+zstd    egg7    egg8    egg9   WINNER (size)");
...
for(const file of files){
  ...
  total++;
  if(verdicts.egg9.ok){
    if(verdicts.egg9.size < art._gzStar) barGz++;
    if(verdicts.egg9.size < verdicts["egg6+zstd"].size) barHybrid++;
    if(verdicts.egg9.size < verdicts.xz.size) barXz++;
  }
}
console.log("-".repeat(136));
console.log("podium: " + Object.entries(tally).sort((a, b) => b[1] - a[1])
  .map(([n, c]) => `${n} x${c}`).join(", "));
console.log(`victory ledger (egg9, armor ON, all injuries EXACT, of ${total} files):`);
