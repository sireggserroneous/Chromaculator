//! eggv12 -- the Transmuter (the Remainder campaign).
//!
...
fn main() -> ExitCode {
    ...
    let usage = || {
        eprintln!("usage: eggv12 transmute <file> [-o out.egg12] [--no-armor] [--survive BYTES] [--tier BLK] [--parity T] [--ct triple|incw|none] [--judge]");
        eprintln!("       eggv12 restore <file.egg12 | .egg11 | .egg10 | .egg9 | .egg8> [-o out] [--wound start:len] [--no-doubles]");
        eprintln!("       eggv12 scratch <file> [--len 4096] [--at payload|checks|parity|head|end|<off>] [--survive BYTES] [--tier BLK] [--parity T]");
        eprintln!("       eggv12 audit [--full]   -- the geometry audit, counts printed");
        ExitCode::from(2)
    };
    ...
    match cmd {
        "transmute" => {
            ...
            let dst = get("-o").unwrap_or(format!("{}.egg12", path));
            ...
            match dearmor(&cont, &[], false) {
                Ok(o) => {
                    ...
                    println!(
                        ...
                        p.total, p.parity, p.ct, p.sites, p.total as f64 / 4096.0, promise(g)
                    );
                    ExitCode::SUCCESS
                }
                Err(e) => {
...
mod pipeline_tests {
    ...
    fn stereo_member_full_pipeline_round_trip() {
        ...
        for fid in [filter::FILTER_MS1, filter::FILTER_MS2] {
            let (inner, model, id, param) = filtered_transmute(&wav, Some((fid, 2)));
            assert_eq!((id, param), (fid, 2));
            ...
            let cont = armor(&inner, rib.blk, rib.t, rib.mode, ex);
            let (back, _) = restore_container(&cont, &[], true).expect("stereo member restore");
