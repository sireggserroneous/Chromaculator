//! armor.rs -- eggv12 armor v4: THE REMAINDER.
//!
...
pub fn promise(g: &Geom) -> String {
    ...
    if g.mode == CtMode::Absent {
        return format!(
            "placement none: no residue table, the {} interleaved codewords locate blind wounds jointly (Krachkovsky-Lee 1997). NAMED (by address or truncation): any {} squares ({} B) anywhere, CERTAIN; BLIND: any {} squares CERTAIN (Berlekamp-Massey), up to {} located jointly (succeeds iff the error rows are independent -- overwhelming for compressed squares; failure REFUSES with the number), {} blind REFUSES; contiguous >= {} B blind; price {} B = {} parity + 0 CT + {} sites (floor 4,096; {:.2}x)",
            g.blk / 2,
            g.t,
            commas(g.t * g.blk),
            g.t / 2,
            g.t.saturating_sub(1),
            g.t,
...
fn write_header(g: &Geom, out: &mut [u8]) {
    out[0..4].copy_from_slice(b"EG12");
    out[4] = 6;
...
fn parse_header(b: &[u8]) -> Option<(Geom, bool)> {
    if b.len() < HDR || &b[0..4] != b"EG12" || b[4] != 6 {
        return None;
    }
...
impl Decoder<'_> {
    ...
    fn decode(&self, e: &BTreeSet<usize>, tally: &mut Tally, depth: usize) -> Result<Vec<u8>, String> {
        ...
        Err(format!(
            "beyond capacity: {} dead squares located, {} codewords still inconsistent (>= {} more damaged squares the syndromes cannot place); capacity {} (t)",
            e.len(),
            bad.len(),
            m.max(1),
            t
    ...
    /// choose the k true positions among `found` (k + false positives)
    fn confirm_subset(&self, e: &BTreeSet<usize>, found: &[usize], k: usize, synd: &[u16], bad: &[usize]) -> Option<Vec<usize>> {
        if found.len() < k || k == 0 || found.len() > k + 8 {
            return None;
        }
        if found.len() == k {
            return if self.consistent(e, found, synd, bad) { Some(found.to_vec()) } else { None };
        }
        let mut idx: Vec<usize> = (0..k).collect();
        loop {
            let sub: Vec<usize> = idx.iter().map(|&i| found[i]).collect();
            if self.consistent(e, &sub, synd, bad) {
                return Some(sub);
            }
            let mut i = k;
            loop {
