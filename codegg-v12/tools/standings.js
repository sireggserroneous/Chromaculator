...
const EGG12 = path.join(here, "..", "target", "release", "eggv12" + (process.platform === "win32" ? ".exe" : ""));
...
function attempt(name, damaged, at, len, kind, src){
  try {
    ...
    else if(name === "egg7" || name === "egg8" || name === "egg9" || name === "egg10" || name === "egg11" || name === "egg12"){
      const exe = name === "egg12" ? EGG12 : name === "egg11" ? EGG11 : name === "egg10" ? EGG10 : name === "egg9" ? EGG9 : name === "egg8" ? EGG8 : EGG7;
...
const NAMES = ["gzip", "zstd", "brotli", "xz", "egg6", "egg6+zstd", "egg7", "egg8", "egg9", "egg10", "egg11", "egg12"];
...
for(const file of files){
  ...
  total++;
  if(verdicts.egg12.ok){
    if(verdicts.egg12.size < art._gzStar) barGz++;
    if(verdicts.egg12.size < verdicts["egg6+zstd"].size) barHybrid++;
    if(verdicts.egg12.size < verdicts.xz.size) barXz++;
    if(verdicts.egg12.size <= Math.min(verdicts.egg8.size, verdicts.egg9.size, verdicts.egg10.size, verdicts.egg11.size)) barMin++;
  }
}
console.log("-".repeat(180));
console.log("podium: " + Object.entries(tally).sort((a, b) => b[1] - a[1])
  .map(([n, c]) => `${n} x${c}`).join(", "));
console.log(`victory ledger (egg12, armor ON, all injuries EXACT, of ${total} files):`);
