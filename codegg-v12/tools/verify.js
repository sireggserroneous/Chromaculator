...
const SP = process.env.EGG_TMP || fs.mkdtempSync(path.join(os.tmpdir(), "egg12sp-"));
const EGG9 = path.join(here, "..", "target", "release", "eggv12" + (process.platform === "win32" ? ".exe" : ""));
...
for(const file of process.argv.slice(2)){
  ...
  cp.execSync(`"${EGG9}" transmute "${file}" -o "${SP}/w.egg12"`, {stdio: "pipe"});
  const art = fs.readFileSync(`${SP}/w.egg12`);
  const short = file.split(/[\\/]/).pop();
  console.log(`${short}  (egg12, ${(100 * art.length / src.length).toFixed(1)}% of original)`);
