...
//!   * `v7_figures` -- **C1**, and it is the gate this round rests on. v6
...
fn read_record(slug: &str) -> Result<String, String> {
    let p = repo_root().join("eggSo-v7").join(format!("measured-{slug}.json"));
    std::fs::read_to_string(&p).map_err(|e| format!("{}: {e}", p.display()))
}

/// L1. v8 copies v7's modules, so a copy can drift silently. This recomputes
/// v7's headline figures with v8's copies and compares them against v7's
/// committed `measured-*.json`:
///
///   * the characterisation sweep -- 409 cases, 0 disagreements;
///   * the three residues of the closed form;
///   * v7's own 5-of-5 pin record.
///
/// No node. If a record file is missing the pin reports SKIPPED loudly.
pub fn v7_figures() -> PinResult {
    const NAME: &str = "the copy vs v7's record";
    let mut checked = 0usize;
    let mut mismatches = 0usize;
    let mut check = |cond: bool| {
        checked += 1;
    ...
    let thirds = match read_record("thirds") {
        Ok(t) => t,
        Err(e) => return PinResult::skipped(NAME, e),
    };
    let mut s = Scan::new(&thirds);
    let cases = s.after("\"cases\"");
    let dis = s.after("\"disagreements\"");
    check(cases == Some(409.0));
    check(dis == Some(0.0));

    // recomputed here with v8's copies rather than read
    let ns: Vec<usize> = (8..=36).collect();
    let ls: Vec<usize> = (3..=18).collect();
    let rows = crate::thirds::characterise(&ns, &ls);
    check(rows.len() == 409);
    check(rows.iter().all(|r| r.agrees()));

    // and the closed form at each residue, independently
    check(crate::thirds::linear_verdict(32, 12) && !crate::thirds::linear_verdict(30, 12));
    check(crate::thirds::linear_verdict(30, 13) && crate::thirds::linear_verdict(31, 13));
    check(!crate::thirds::linear_verdict(30, 8) && crate::thirds::linear_verdict(31, 8));
    ...
    let mut seen600 = false;
    for _ in 0..8 {
        if !s.seek("\"checked\"") {
            break;
        }
        let c = s.num();
        check(s.after("\"mismatches\"") == Some(0.0));
        if c == Some(600.0) {
            seen600 = true;
        }
    }
...
mod tests {
    ...
    /// THE NEGATIVE CONTROL. A pin that cannot fail is not a pin: perturb
    /// v7's committed record in memory and the figure must read back
    /// perturbed, which proves the reader addresses the field it believes it
    /// addresses.
    #[test]
    fn a_perturbed_record_reads_back_perturbed() {
        let Ok(real) = read_record("thirds") else {
            return; // v7's record is not on disk; v7_figures reports SKIPPED
        };
        let cases = |t: &str| Scan::new(t).after("\"cases\"");
        assert_eq!(cases(&real), Some(409.0), "v7 recorded 409 characterisation cases");
        let faked = real.replacen("\"cases\": 409", "\"cases\": 999", 1);
        assert_ne!(faked, real, "the perturbation did not apply");
        assert_eq!(cases(&faked), Some(999.0), "the reader ignored a change it should have seen");
    }
    ...
    #[test]
    fn the_v7_figures_pin_checks_something_and_agrees() {
        let r = v7_figures();
